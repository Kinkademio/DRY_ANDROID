...
public class button_menu : MonoBehaviour
{
    ...
    {
        GameObject newHouse =  Instantiate(housePrefab, spawnPointsHouses[spawn].transform.position, Quaternion.identity, parent);
        House newHouseClass = newHouse.GetComponent<House>();
    ...
    private void GenaretionLetter()
    {
        ...
        while (i < Letters.Length)
        {
            ...
            if (pointLetters.Contains(spawnLetter))
            {
                ...
                GameObject newLetter = Instantiate(letterPrefab, spawnPointsLetters[spawnLetter].transform.position, Quaternion.identity, parent);
                House newLetterClass = newLetter.GetComponent<House>();
...
//Метод генерации домов и букв
    ...
    {
        if (house.Length == HouseHARD.Length) winnigNumber = house.Length - 3;
